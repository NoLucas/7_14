...
> 헤더에 "바리스타" 메뉴를 추가해(기존 "라떼아트" 갤러리 링크와는 별개) 새 "바리스타 소개" 섹션으로 스크롤 이동한다. 라떼아트를 담당하는 바리스타의 사진(플레이스홀더 아이콘)과 이력/소개글을 보여준다. 15단계에서 만든 영상 갤러리 섹션은 그대로 유지되며, 헤더에서 별도 링크로 연결되진 않지만 스크롤하면 자연스럽게 보인다. **알려진 사항**: 바리스타 사진/이력은 실제 인물 정보가 아직 없어 플레이스홀더(아이콘 + 예시 텍스트)로 채워짐 — 실제 내용이 준비되면 교체 필요.

- [ ] `frontend/index.html` — 헤더 nav에 "바리스타" 메뉴 추가(`#baristaProfile` 앵커), 바리스타 소개 섹션 마크업 추가(사진 아이콘 + 이름/직함/경력/소개글, 플레이스홀더 내용)
- [ ] `frontend/index.css` — 바리스타 소개 섹션 스타일 (사진+텍스트 카드 레이아웃, 반응형)
